...
| **호환 방법** | `CLAUDE.md`에서 `@AGENTS.md` import (`ln -s AGENTS.md CLAUDE.md`는 비권장 — [AGENTS-Guide.md](./AGENTS-Guide.md) 참고) | - |
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "f=$(jq -r '.tool_input.file_path // \"\"'); if echo \"$f\" | grep -qE '\\.(ts|tsx)$'; then npx tsc --noEmit --pretty 2>&1 | head -20; fi"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cmd=$(jq -r '.tool_input.command // \"\"'); if echo \"$cmd\" | grep -qE 'rm -rf|DROP TABLE|git push --force'; then echo 'BLOCKED: 위험한 명령어가 감지되었습니다.' >&2; exit 2; fi"
          }
        ]
      }
    ]
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "cmd=$(jq -r '.tool_input.command // \"\"'); if echo \"$cmd\" | grep -qE 'rm -rf|DROP TABLE|TRUNCATE|git push --force'; then echo 'BLOCKED: dangerous command detected' >&2; exit 2; fi"
          }
        ]
      }
    ],
